...
export default async function FulfillmentDailyLogLinkPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const result = await getSupplyRequestDailyLogLinkContext(id, "FULFILLMENT");
  ...
  return <SupplyRequestDailyLogLinkForm context={result.context} setAction={setSupplyRequestDailyLogLinkAction.bind(null, result.context.supplyRequestId, "FULFILLMENT")} removeAction={removeSupplyRequestDailyLogLinkAction.bind(null, result.context.supplyRequestId, "FULFILLMENT")} />;
}
