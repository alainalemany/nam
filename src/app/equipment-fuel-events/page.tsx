    ...
    <main className="page-stack">
      ...
      <section className="panel table-panel" aria-labelledby="fuel-events-heading">
        ...
        {events.length === 0 ? (
          ...
          <div className="table-wrap"><table><thead><tr><th>Date and time</th><th>Equipment</th><th>Gas Station</th><th>Fuel</th><th>Total gallons</th><th>Total cost</th><th>Actions</th></tr></thead><tbody>{events.map((event) => <tr key={event.id}><td>{displayEquipmentFuelDate(event.operationalWorkDate)}<span className="subtle">{event.eventTime} local</span></td><td>{event.equipmentDisplayName}<span className="subtle">{event.equipmentNumber ?? "No Equipment number"}</span></td><td>{event.gasStationNameSnapshot ?? "Not recorded"}<span className="subtle">{event.gasStationCitySnapshot ? `${event.gasStationCitySnapshot}${event.gasStationStateSnapshot ? `, ${event.gasStationStateSnapshot}` : ""}` : "Legacy event"}</span></td><td>{fuelTypeLabel(event.fuelType)}</td><td>{formatFuelGallons(event.totalGallons)}</td><td>{formatFuelCurrency(event.totalCost)}</td><td className="action-cell"><Link className="table-action" href={`/equipment-fuel-events/${event.id}`}>View</Link><Link className="table-action" href={`/equipment-fuel-events/${event.id}/edit`}>Correct</Link></td></tr>)}</tbody></table></div>
        )}
      </section>
    </main>
  );
