...
- Supply Requests, with Phase 26.1 product discovery, Phase 26.2 feature
  ...
  cancellation, and Phase 26.7 Correct Request and full immutable version
  review are implemented and accepted. Operators can record an already
  submitted Supply Request in NAM, manage Supply Items and supervisors, review
  ...
  Cancellation and correction do not modify the corporate request. Canonical
  Supply Request history and filtering are the next candidate and require
  separate authorization. Canonical filtering, Daily Log links, and Day View
  participation remain unavailable. Infrastructure and the operational pilot
  remain outside this feature series.
...
| Supply Requests | Preserves records of requests already submitted through the corporate system, with permanent identity, ordered catalog items, lifecycle, immutable history, and bounded Daily Log and Day View context. | Equipment reference data; Phase 26.1 product decisions; boundary assessment: `docs/architecture/equipment-operations.md`; Approved feature architecture: `docs/architecture/features/supply-requests.md`. | Phases 26.1 through 26.7 complete and accepted; create, current detail, reference management, fulfillment, NAM-only cancellation, Correct Request, Correction History, and read-only immutable version review exist; canonical history and filtering are the next candidate and require separate authorization; Daily Log links and Day View remain unavailable |
