...
| Real operational use depends on pilot authorization gates | ADR-019 approves a managed private overlay. Tailscale is partially configured but the private boundary is not accepted; private access and administrator recovery, public exposure removal, immutable current deployment, migration parity, reviewed reference data, current-schema recovery, and pilot scope must all pass before real pilot entry. See `docs/infrastructure/controlled-pilot-readiness-rebaseline.md` and `docs/infrastructure/operational-pilot-runbook.md`. |
